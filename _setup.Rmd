...
docVersions <- sapply(docVersions, function(x) {
    paste0("document.writeln(\"<option value=\\\"../docs_", x, "/index.html\\\">", x, "</option>\");")
})
versions <- readLines("docs/_version.js")
version_ind <- grep("<option", versions)
versions <- c(
    versions[1:(version_ind[1] - 1)],
    docVersions,
    versions[(max(version_ind) + 1):length(versions)]
)
writeLines(versions, "docs/_version.js")
